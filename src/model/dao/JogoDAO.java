...
public class JogoDAO implements ListagemDAO<Jogo>{
    ...
    // CADASTRAR
    // =========================
    public void cadastrarJogoObjeto(Jogo jogo) {

        String sql = "INSERT INTO jogo "
                + "(idMidia, nome, preco, classIndicativa, genero, qtdEstoque, capa, tipo, id, isOnline) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try {
            ...
            stmt.setInt(1, jogo.getIdMidia());
            stmt.setString(2, jogo.getNome());
            stmt.setDouble(3, jogo.getPreco());
            stmt.setString(4, jogo.getClassIndicativa());
            stmt.setString(5, jogo.getGenero());
            stmt.setInt(6, jogo.getQtdEstoque());
            stmt.setString(7, jogo.getCapa());
            stmt.setString(8, jogo.getTipo());
            stmt.setInt(9, jogo.getIdJogo());
            stmt.setBoolean(10, jogo.getIsOnline());

            stmt.executeUpdate();

        } catch (SQLException e) {
            System.out.println("Erro ao cadastrar jogo: " + e);
        } 
    }
    ...
    // ALTERAR
    // =========================
    public void alterarJogoObjeto(int idMidia, Jogo jogo) {
        try (Connection con = ConectaBD.conexao()) {
            PreparedStatement stMidia = con.prepareStatement(
    ...
    public Jogo buscarJogoPorId(int id) {
        ...
        try (Connection conecta = ConectaBD.conexao()) {
            ...
            if (rs.next()) {
                ...
                jogo = new Jogo(idMidia, nome, preco, classIndicativa, genero, qtdEstoque, capa, tipo, id, isOnline);
            }
