...
public class AlbumDAO implements ListagemDAO<Album> {
	...
	// ✅ NOVO MÉTODO - Aceita objeto Album ao invés de usar JOptionPane
	public void cadastrarAlbumObjeto(Album album) {
	...
	// ✅ NOVO MÉTODO - Aceita objeto Album para atualização
	public void alterarAlbumObjeto(int idAlbum, Album album) {
		try (Connection conecta = ConectaBD.conexao()) {
			String artistaString = String.join(",", album.getArtistas());
			String musicaString = String.join(",", album.getMusicas());
	...
	public void alterarAlbum(int idAlbum) {
		try (Connection conecta = ConectaBD.conexao()) {
			...
			// Chama o novo método
			alterarAlbumObjeto(idAlbum, album);
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Erro ao conectar ao banco de dados: " + e.getMessage());
        }
    }
	...
	// ✅ MÉTODO PARA BUSCAR ÁLBUM POR ID (NECESSÁRIO PARA EDIÇÃO)
	public Album buscarAlbumPorId(int id) {
	    ...
	    try (Connection conecta = ConectaBD.conexao()) {
	        String sql = "SELECT m.idMidia, m.nome, m.preco, m.classIndicativa, m.genero, m.qtdEstoque, m.capa, m.tipo, " +
	                     "a.idAlbum, a.anoLancamento, a.artista, a.musica " +
	                     "FROM midia m " +
	                     "INNER JOIN album a ON m.idMidia = a.idAlbum " +
	                     "WHERE m.idMidia = ?";
	        ...
	        if (rs.next()) {
	            ...
	            ArrayList<String> artistas = new ArrayList<>();
	            if (artistaBD != null && !artistaBD.isEmpty()) {
	                String[] artistasArray = artistaBD.split(",");
	            ...
	            ArrayList<String> musicas = new ArrayList<>();
	            if (musicaBD != null && !musicaBD.isEmpty()) {
	                String[] musicasArray = musicaBD.split(",");
	                for (String musica : musicasArray) {
	                    musicas.add(musica.trim());
	                }
	            }
	            
	            album = new Album(idMidia, nome, preco, classIndicativa, genero, qtdEstoque, 
	                            capa, tipo, idAlbum, anoLancamento, artistas, musicas);
	        }
