...
public class MidiaDAO {
    ...
    public java.util.List<Midia> listarTodasMidias() {
        java.util.List<Midia> lista = new java.util.ArrayList<>();
        try (Connection conecta = ConectaBD.conexao();
             PreparedStatement st = conecta.prepareStatement(
                 "SELECT * FROM midia ORDER BY idMidia DESC")) {
            try (ResultSet rs = st.executeQuery()) {
                while (rs.next()) {
                    lista.add(new Midia(
                        rs.getInt("idMidia"),
            ...
            JOptionPane.showMessageDialog(null, "Erro ao listar mídias: " + e.getMessage());
        }
        return lista;
    }
